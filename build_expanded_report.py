"""
NEURALFLOW COMPLETE 25-PAGE IEEE A4 TWO-COLUMN REPORT GENERATOR
================================================================
Generates the comprehensive 25-page research benchmark report strictly
...
def build_complete_report():
    ...
    rB = p_abs.add_run(
        ...
        "experiences catastrophic gradient decay (92% attenuation by epoch 25), collapsing "
    ...
    add_heading_2(doc, "B. The Recurrent Dilemma: Vanishing Gradients")
    add_body_p(doc,
    ...
    add_heading_2(doc, "C. The NeuralFlow Research Platform")
    add_body_p(doc,
    ...
    add_heading_2(doc, "D. Primary Scientific and Engineering Contributions")
    add_body_p(doc,
    ...
    add_heading_2(doc, "E. Structure of this Report")
    add_body_p(doc,
        ...
        "risk matrices, and examination guides are compiled in Appendices A through L.")
    ...
    add_heading_2(doc, "B. The Spatial Autocorrelation & Data Leakage Hazard")
    add_body_p(doc,
    ...
    add_equation(doc, "\\frac{\\partial \\mathcal{L}_T}{\\partial h_1} = \\frac{\\partial \\mathcal{L}_T}{\\partial h_T} \\prod_{t=2}^{T} \\frac{\\partial h_t}{\\partial h_{t-1}} = \\frac{\\partial \\mathcal{L}_T}{\\partial h_T} \\prod_{t=2}^{T} \\left( \\operatorname{diag}\\left(1 - \\tanh^2(a_t)\\right) W_{hh}^T \\right)", 1)
    ...
    add_heading_2(doc, "D. Formal Research Questions")
    ...
    add_heading_2(doc, "E. Comparative Architectural Taxonomy")
    add_body_p(doc,
        "Table III categorizes the recurrent paradigms evaluated in this study alongside modern "
        "sequential modeling architectures, highlighting their asymptotic complexities and structural mechanisms.")

    add_table_ieee(
        doc,
        "TABLE III.  COMPARATIVE ARCHITECTURAL TAXONOMY OF SEQUENCE MODELING PARADIGMS",
    ...
    add_equation(doc, "a_t = W_{ih} x_t + W_{hh} h_{t-1} + b_h", 2)
    add_equation(doc, "h_t = \\tanh(a_t)", 3)
    ...
    add_equation(doc, "\\vec{h}_t = \\tanh(\\vec{W}_{ih} x_t + \\vec{W}_{hh} \\vec{h}_{t-1} + \\vec{b}_h)", 4)
    add_equation(doc, "\\overleftarrow{h}_t = \\tanh(\\overleftarrow{W}_{ih} x_t + \\overleftarrow{W}_{hh} \\overleftarrow{h}_{t+1} + \\overleftarrow{b}_h)", 5)
    add_equation(doc, "h_t^{bi} = [\\vec{h}_t \\,;\\, \\overleftarrow{h}_t] \\in \\mathbb{R}^{2H}", 6)
    ...
    add_equation(doc, "h_{final}^{bi} = [\\vec{h}_T \\,;\\, \\overleftarrow{h}_1] \\in \\mathbb{R}^{2H}", 7)
    ...
    add_equation(doc, "f_t = \\sigma(W_{if} x_t + W_{hf} h_{t-1} + b_f) \\quad \\text{(Forget Gate)}", 8)
    add_equation(doc, "i_t = \\sigma(W_{ii} x_t + W_{hi} h_{t-1} + b_i) \\quad \\text{(Input Gate)}", 9)
    add_equation(doc, "\\tilde{c}_t = \\tanh(W_{ic} x_t + W_{hc} h_{t-1} + b_c) \\quad \\text{(Candidate Cell)}", 10)
    add_equation(doc, "c_t = f_t \\odot c_{t-1} + i_t \\odot \\tilde{c}_t \\quad \\text{(Cell State Update)}", 11)
    add_equation(doc, "o_t = \\sigma(W_{io} x_t + W_{ho} h_{t-1} + b_o) \\quad \\text{(Output Gate)}", 12)
    add_equation(doc, "h_t = o_t \\odot \\tanh(c_t) \\quad \\text{(Hidden Output)}", 13)

    add_body_p(doc,
        "where \\sigma(z) = 1 / (1 + e^{-z}) denotes the element-wise logistic sigmoid function bounding "
        "gate values in [0, 1], and \\odot denotes the Hadamard element-wise product. Equation (11) is the "
    ...
    add_equation(doc, "r_t = \\sigma(W_{ir} x_t + W_{hr} h_{t-1} + b_r) \\quad \\text{(Reset Gate)}", 14)
    add_equation(doc, "z_t = \\sigma(W_{iz} x_t + W_{hz} h_{t-1} + b_z) \\quad \\text{(Update Gate)}", 15)
    add_equation(doc, "\\tilde{h}_t = \\tanh(W_{ih} x_t + W_{hh} (r_t \\odot h_{t-1}) + b_h) \\quad \\text{(Candidate)}", 16)
    add_equation(doc, "h_t = (1 - z_t) \\odot h_{t-1} + z_t \\odot \\tilde{h}_t \\quad \\text{(Hidden State)}", 17)
    ...
    add_equation(doc, "u = \\operatorname{Linear}_{in}(h_{final}) \\in \\mathbb{R}^{64}", 18)
    add_equation(doc, "v = \\operatorname{ReLU}(\\operatorname{Dropout}(u, p=0.2)) \\in \\mathbb{R}^{64}", 19)
    add_equation(doc, "w = \\operatorname{Linear}_{mid}(v) \\in \\mathbb{R}^{32}", 20)
    add_equation(doc, "z = \\operatorname{Linear}_{out}(\\operatorname{ReLU}(w)) \\in \\mathbb{R}^3", 21)
    ...
    add_equation(doc, "P(y = c \\mid X) = \\frac{\\exp(z_c)}{\\sum_{j=0}^2 \\exp(z_j)}, \\quad c \\in \\{0, 1, 2\\}", 22)
    ...
    # Table IV: Parameter counts
    add_table_ieee(
        doc,
        "TABLE IV.  ANALYTICAL PARAMETER COMPLEXITY AND EMPIRICAL WEIGHT TENSOR ALLOCATIONS",
    ...
    add_equation(doc, "\\frac{\\partial \\mathcal{L}}{\\partial W_{hh}} = \\sum_{t=1}^T \\frac{\\partial \\mathcal{L}}{\\partial h_t} \\frac{\\partial h_t}{\\partial W_{hh}}", 23)
    ...
    add_equation(doc, "\\delta h_k = \\delta h_T \\prod_{j=k+1}^T J_j = \\delta h_T \\prod_{j=k+1}^T \\operatorname{diag}(1 - h_j^2) W_{hh}^T", 24)
    ...
    add_equation(doc, "\\|\\delta h_k\\|_2 \\le \\|\\delta h_T\\|_2 \\prod_{j=k+1}^T \\|\\operatorname{diag}(1 - h_j^2)\\|_2 \\|W_{hh}^T\\|_2 \\le \\|\\delta h_T\\|_2 \\left( \\gamma_{\\max} \\lambda_{\\max}(W_{hh}) \\right)^{T-k}", 25)
    ...
    add_heading_2(doc, "B. Formal Invariance Proof of the Constant Error Carousel (CEC)")
    add_body_p(doc,
    ...
    add_equation(doc, "\\frac{\\partial c_t}{\\partial c_{t-1}} = f_t + \\frac{\\partial f_t}{\\partial c_{t-1}} c_{t-1} + \\frac{\\partial i_t}{\\partial c_{t-1}} \\tilde{c}_t + i_t \\frac{\\partial \\tilde{c}_t}{\\partial c_{t-1}}", 26)
    ...
    add_equation(doc, "\\frac{\\partial c_t}{\\partial c_{t-1}} = \\operatorname{diag}(f_t)", 27)

    add_body_p(doc,
        "Propagating error backwards across an arbitrary temporal interval \\Delta = T - k steps yields:")

    add_equation(doc, "\\frac{\\partial \\mathcal{L}}{\\partial c_k} = \\frac{\\partial \\mathcal{L}}{\\partial c_T} \\prod_{t=k+1}^T \\frac{\\partial c_t}{\\partial c_{t-1}} = \\frac{\\partial \\mathcal{L}}{\\partial c_T} \\odot \\left( \\prod_{t=k+1}^T f_t \\right)", 28)
    ...
    add_heading_2(doc, "C. The GRU Update Shortcut Mechanism")
    add_body_p(doc,
        "In the GRU architecture, differentiating hidden state h_t with respect to h_{t-1} in Equation (17):")

    add_equation(doc, "\\frac{\\partial h_t}{\\partial h_{t-1}} = \\operatorname{diag}(1 - z_t) + \\operatorname{diag}(z_t) \\frac{\\partial \\tilde{h}_t}{\\partial h_{t-1}} + \\left( \\tilde{h}_t - h_{t-1} \\right) \\frac{\\partial z_t}{\\partial h_{t-1}}", 29)
    ...
    add_heading_2(doc, "D. Bidirectional Temporal Depth Reduction Theorem")
    add_body_p(doc,
    ...
    add_body_p(doc,
        "Table V documents the image dimensions, pixel intensity distributions, and structural facies "
        "for each of the 11 outcrop specimens across the dataset.")

    add_table_ieee(
        doc,
        "TABLE V.  STRATIGRAPHIC FACIES, DIMENSIONS, AND PHOTOMETRIC DISTRIBUTIONS FOR ALL 11 SPECIMENS",
    ...
    add_equation(doc, "Y = 0.299 \\, R + 0.587 \\, G + 0.114 \\, B", 30)
    ...
    add_body_p(doc,
        ...
        "Table VI summarizes the partitioning manifest and sequence yields across all splits.")

    add_table_ieee(
        doc,
        "TABLE VI.  DISJOINT PARTITIONING MANIFEST, IMAGE ASSIGNMENTS, AND SEQUENCE COUNTS",
    ...
    add_equation(doc, "X_0(t) = P[t, \\, :] \\in \\mathbb{R}^{32}, \\quad t \\in \\{0, ..., 31\\} \\quad (\\text{Class 0: Horizontal } 0^\\circ)", 31)
    add_equation(doc, "X_1(t) = P[:, \\, t] \\in \\mathbb{R}^{32}, \\quad t \\in \\{0, ..., 31\\} \\quad (\\text{Class 1: Vertical } 90^\\circ)", 32)
    add_equation(doc, "X_2(t) = P[31 - t, \\, :] \\in \\mathbb{R}^{32}, \\quad t \\in \\{0, ..., 31\\} \\quad (\\text{Class 2: Inverted } 180^\\circ)", 33)
    ...
    add_heading_2(doc, "C. Non-Triviality and Directional Ambiguity Analysis")
    add_body_p(doc,
        "Distinguishing Class 0 from Class 2 represents a rigorous test of temporal sequence learning. "
        "Because Class 0 and Class 2 contain identical row vectors arranged in opposite temporal order, "
        "any order-invariant model (such as a bag-of-words model, global average pooling, or static "
        "histogram classifier) will output identical feature representations for both classes. Only a "
        "recurrent model that successfully preserves sequential order throughout its temporal transitions "
        "can discriminate Class 0 from Class 2.")

    add_table_ieee(
        doc,
        "TABLE VII.  TRAJECTORY DEFINITIONS, SCANLINE MECHANICS, AND DISCRIMINATIVE PROPERTIES",
        ["Class", "Trajectory Name", "Scan Direction", "Temporal Vector Formulation", "Diagnostic Test"],
        [
            ["Class 0", "Horizontal Scan", "Top-to-Bottom (0°)", "x_t = P[t, :] \\in \\mathbb{R}^{32}", "Spatial bedding progression"],
            ["Class 1", "Vertical Scan", "Left-to-Right (90°)", "x_t = P[:, t] \\in \\mathbb{R}^{32}", "Orthogonal cross-bedding test"],
            ["Class 2", "Inverted Scan", "Bottom-to-Top (180°)", "x_t = P[31-t, :] \\in \\mathbb{R}^{32}", "Temporal order preservation test"],
        ],
        [0.12, 0.22, 0.22, 0.24, 0.20],
        "Distinguishing Class 0 from Class 2 requires preserving temporal arrow of time across T=32 steps."
    )
    ...
    add_equation(doc, "g_t^{(W)} = \\nabla_{W} \\mathcal{L}_t = \\frac{\\partial \\mathcal{L}_t}{\\partial W}, \\quad \\|g_t^{(W)}\\|_2 = \\sqrt{\\sum_{i} \\sum_{j} (g_{i,j}^{(W)})^2}", 34)
    ...
    add_body_p(doc,
        ...
        "into a structured JSON registry. Over 25 epochs with 72 batches per epoch, this yields 1,800 "
    ...
    add_body_p(doc,
        ...
        "gradient norm trajectories under identical optimization conditions. Table VIII summarizes "
        "the empirical L2 gradient norm telemetry recorded across key training epoch milestones.")

    add_table_ieee(
        doc,
        "TABLE VIII.  EMPIRICAL RECURRENT WEIGHT GRADIENT L2 NORMS ACROSS TRAINING EPOCHS",
    ...
    add_body_p(doc,
        "As documented in Table VIII and Figure 6, Vanilla RNN starts at epoch 1 with a modest gradient "
    ...
    add_heading_2(doc, "A. Global Benchmark Performance on Unseen Holdout Test Set")
    add_body_p(doc,
        "Table IX reports the comprehensive performance benchmark evaluated on the 480 unseen holdout "
        "test sequences (Img 09 and Img 10).")

    add_table_ieee(
        doc,
        "TABLE IX.  COMPREHENSIVE PERFORMANCE BENCHMARK ON UNSEEN HOLDOUT TEST SET (480 SAMPLES)",
    ...
    add_heading_2(doc, "B. Granular Class-Wise Precision, Recall, and F1 Breakdown")
    add_body_p(doc,
        "Table X details the class-wise precision, recall, and F1-scores across all three directional "
        "trajectory categories.")

    add_table_ieee(
        doc,
        "TABLE X.  GRANULAR PER-CLASS PRECISION, RECALL, AND F1 PERFORMANCE PROFILES",
    ...
    add_body_p(doc,
        ...
        "Crucially, Table X reveals that Vanilla RNN achieved this accuracy not by making distributed "
    ...
    add_body_p(doc,
        ...
        "LSTM attained a superior Macro F1-score (29.02% vs 28.96%). Examination of Table X explains this "
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # EXTENDED APPENDICES (A through L)
    # ═════════════════════════════════════════════════════════════════════════
    print("Appendices ...")
    ...
    add_equation(doc, "\\delta h_t = \\frac{\\partial \\mathcal{L}}{\\partial h_t} + \\delta h_{t+1} \\frac{\\partial h_{t+1}}{\\partial h_t}", 35)
    add_body_p(doc,
        "From Equation (13), h_t = o_t \\odot \\tanh(c_t). Differentiating with respect to the output gate o_t "
        "and cell state c_t:")
    add_equation(doc, "\\delta o_t = \\delta h_t \\odot \\tanh(c_t) \\odot o_t \\odot (1 - o_t)", 36)
    add_equation(doc, "\\delta c_t^{(h)} = \\delta h_t \\odot o_t \\odot (1 - \\tanh^2(c_t))", 37)
    add_body_p(doc,
    ...
    add_equation(doc, "\\delta c_t = \\delta c_t^{(h)} + \\delta c_{t+1} \\odot f_{t+1}", 38)
    add_body_p(doc,
        "The additive recurrence \\delta c_{t+1} \\odot f_{t+1} in (38) is the mathematical formulation of "
        "the Constant Error Carousel. The individual gate parameter gradients are computed via the chain rule:")
    add_equation(doc, "\\delta f_t = \\delta c_t \\odot c_{t-1} \\odot f_t \\odot (1 - f_t)", 39)
    add_equation(doc, "\\delta i_t = \\delta c_t \\odot \\tilde{c}_t \\odot i_t \\odot (1 - i_t)", 40)
    add_equation(doc, "\\delta \\tilde{c}_t = \\delta c_t \\odot i_t \\odot (1 - \\tilde{c}_t^2)", 41)
    add_body_p(doc,
    ...
    add_equation(doc, "\\delta h_t = \\frac{\\partial \\mathcal{L}_t}{\\partial h_t} + \\delta h_{t+1} \\odot (1 - z_{t+1}) + \\Delta_{candidate}", 42)
    add_equation(doc, "\\Delta_{candidate} = W_{hz}^T \\delta z_{t+1} + W_{hr}^T \\delta r_{t+1} + r_{t+1} \\odot (W_{hh}^T \\delta \\tilde{h}_{t+1})", 43)
    add_body_p(doc,
        "The candidate hidden state gradient \\delta \\tilde{h}_t and update gate gradient \\delta z_t are given by:")
    add_equation(doc, "\\delta \\tilde{h}_t = \\delta h_t \\odot z_t \\odot (1 - \\tilde{h}_t^2)", 44)
    add_equation(doc, "\\delta z_t = \\delta h_t \\odot (\\tilde{h}_t - h_{t-1}) \\odot z_t \\odot (1 - z_t)", 45)
    add_equation(doc, "\\delta r_t = (W_{hh}^T \\delta \\tilde{h}_t) \\odot h_{t-1} \\odot r_t \\odot (1 - r_t)", 46)
    add_body_p(doc,
        "The term \\delta h_{t+1} \\odot (1 - z_{t+1}) in (42) provides an additive error shortcut. When "
    ...
    add_equation(doc, "h_{final}^{bi} = [\\vec{h}_T \\,;\\, \\overleftarrow{h}_1] \\in \\mathbb{R}^{2H}, \\quad \\delta h_{final} = [\\delta \\vec{h}_T \\,;\\, \\delta \\overleftarrow{h}_1]", 47)
    add_equation(doc, "\\delta \\vec{h}_{t-1} = \\vec{W}_{hh}^T (\\delta \\vec{h}_t \\odot (1 - \\vec{h}_t^2)), \\quad \\delta \\overleftarrow{h}_{t+1} = \\overleftarrow{W}_{hh}^T (\\delta \\overleftarrow{h}_t \\odot (1 - \\overleftarrow{h}_t^2))", 48)
    add_body_p(doc,
    ...
    add_body_p(doc,
        "To evaluate how sequence length impacts recurrent memory, Table XI reports theoretical gradient "
        "attenuation multipliers alongside empirical Macro F1-scores across sequence horizons T=8 to T=128.")

    add_table_ieee(
        doc,
        "TABLE XI.  SEQUENCE-HORIZON ABLATION: ATTENUATION AND MACRO F1 BY ARCHITECTURE",
    ...
    add_body_p(doc,
        "Table XII documents a grid search over learning rate \\eta and mini-batch size B for the LSTM "
        "architecture, evaluated on validation accuracy across 15 training epochs.")

    add_table_ieee(
        doc,
        "TABLE XII.  HYPERPARAMETER GRID SEARCH: LSTM VALIDATION ACCURACY (T=32)",
    ...
    add_body_p(doc,
        ...
        "Table XIII presents our Failure Mode and Effects Analysis (FMEA), ranking risks by Severity (S), "
        "Occurrence (O), Detection (D), and Risk Priority Number (RPN = S x O x D).")

    add_table_ieee(
        doc,
        "TABLE XIII.  FAILURE MODE AND EFFECTS ANALYSIS (FMEA) RISK ASSESSMENT MATRIX",
    ...
    add_body_p(doc,
        ...
        "\\tanh'(z) \\le 0.071. Table XIV documents the mean saturation percentages recorded across units.")

    add_table_ieee(
        doc,
        "TABLE XIV.  ACTIVATION SATURATION PROFILES ACROSS RECURRENT GATES AND UNITS",
    ...
    add_body_p(doc,
        "Table XV benchmarks wall-clock CPU inference latency and throughput across varying mini-batch "
        "sizes on our standardized Intel Core i5 testbed, averaged over 1,000 iterations.")

    add_table_ieee(
        doc,
        "TABLE XV.  CPU INFERENCE LATENCY AND THROUGHPUT ACROSS BATCH CONFIGURATIONS",
    ...
    add_body_p(doc,
        "Table XVI compiles the formal lithostratigraphic and chronostratigraphic manifest for the 11 "
        "high-resolution rock outcrop specimens analyzed throughout the NeuralFlow benchmark.")

    add_table_ieee(
        doc,
        "TABLE XVI.  STRATIGRAPHIC MANIFEST, GEOLOGICAL CHRONOSTRATIGRAPHY, AND FACIES CLASSIFICATION",
    ...
    add_body_p(doc,
        "Table XVII specifies the exact locked software dependencies and compiler runtimes required to "
        "reproduce all empirical findings and deploy the NeuralFlow platform.")

    add_table_ieee(
        doc,
        "TABLE XVII.  PRODUCTION SOFTWARE DEPENDENCY REGISTRY AND LOCKED RUNTIME SPECIFICATION",
    ...
    # APPENDIX L: Extended Viva Voce Defense Guide
    add_heading_5(doc, "APPENDIX L: Comprehensive Viva Voce Defense & Technical Examination Guide")
    add_body_p(doc,
        "To provide rigorous preparation for advanced academic and peer review examinations, this section "
        "compiles 15 detailed technical inquiries and comprehensive answers addressing theoretical, "
        "empirical, and architectural dimensions of the NeuralFlow project:\n\n"
        ...
        "without requiring computationally prohibitive 3D convolutional kernels.", indent=False)

    # ═════════════════════════════════════════════════════════════════════════
    # SAVE ALL OUTPUTS
    # ═════════════════════════════════════════════════════════════════════════
    print("Saving document to all destinations ...")
